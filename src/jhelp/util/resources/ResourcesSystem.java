 ...
 * If resources are embed inside a jar only read operations are availables, if there are in outside directory read and
 * write
 * operations are possible.<br>
 * The path separator is /, what ever the system you are, conversions are made for you.<br>
...
public class ResourcesSystem
{
     ...
     *
     * @param resources {@link Resources} reference to find resources
     * @param rootFile  Outside root directory
     */
     ...
     *
     * @param resources {@link Resources} reference to find resources
     * @param jarFile   Jar file that contains resources
     * @param rootPath  Root opath inside the jar
     */
     ...
     * @return {@code true} if directory created (or allready exists). {@code false} if failed to create (Two situations,
     * inside
     ...
     * @return {@code true} if file created (or allready exists). {@code false} if failed to create (Two situations,
     * inside a jar
     ...
     *
     * @param resourceElement Resource elemnt to have its parent directory
     ...
     *
     * @param resourceElement Resource elemnt to have its children
    ...
    public OutputStream obtainOutputStream(final ResourceFile resourceFile) throws IOException
    {
        ...
        if (!UtilIO.createFile(file))
        {
            throw new IOException(
                    "Failed to create new resource : " + resourceFile.getPath() + "[" + file.getAbsolutePath() + "]");
        }
    ...
    public File obtainReaFile(final ResourceElement resourceElement)
    {
        ...
        if (ResourcesSystem.ROOT.equals(resourceElement))
        {
