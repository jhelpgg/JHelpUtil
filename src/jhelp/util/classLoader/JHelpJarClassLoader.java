        ...
        extends ClassLoader
{
     ...
     *
     * @param classLoader Reference class loader
     */
     ...
     * @throws IOException If file is not a jar file
     */
     ...
     * Add a jar file
     *
     * @param name Jar file path
     * @throws IOException If path is not a jar file
     */
    ...
    public HashSet<String> listOfDirectInternal(final String name)
    {
        ...
        for (final JarFile jarFile : this.arrayList)
        {
            ...
            while (enumeration.hasMoreElements())
            {
                ...
                if (nameEntry.endsWith(".class") && (nameEntry.startsWith(plus)) && (nameEntry.indexOf('$', index) < 0))
                {
                    ind = nameEntry.lastIndexOf('.');
                    list.add(nameEntry.substring(0, ind)
                                      .replace('/', '.'));
                }
            }
        }
     ...
     * Load a class
     *
     * @param name Class complete name
     * @return Loaded class
     ...
     *
     * @param name    Class complete name
     * @param resolve Indicates if have to resolve
     * @return Loaded class
     * @throws ClassNotFoundException If file not exists in one of jars and can't be retrieve by default class loader
     ...
     *
     * @param name Resource complete name
     ...
     * @throws IOException On creating list issus
     ...
     *
     * @param spy Spy/listener to register
     */
