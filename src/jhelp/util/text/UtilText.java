...
public final class UtilText
{
    ...
    public static String[] capture(final String pattern, final String text)
    {
        ...
        int          nb            = 0;
        for (char aCharPattern : charPattern)
        {
            if (aCharPattern == '?')
            {
                nb++;
        ...
        while ((indexPattern < lengthPattern) && (indexText < lengthText))
        {
            if (waiting)
            {
                if ((charText[indexText] == waitFor) && (count1 == 0) && (count2 == 0) && (count3 == 0) && (bool1) &&
                        (bool2))
                {
                    waiting = false;
    ...
    public static String colorText(final int color)
    {
        ...
        {
            characters[i] = Integer.toHexString((color >> shift) & 0xF)
                                   .charAt(0);
        }
     ...
     * @param caseSensitive  Indicates if for word comparison case is important or not
     * @param wordDelimiters List of delimiters of words (Each characters of this string are see as separator and only
     *                       them)
    ...
    public static int computeDistance(String text1, String text2, final boolean caseSensitive, final String wordDelimiters)
    {
        if (text1 == null)
        {
            throw new NullPointerException("text1 MUST NOT be null");
        }
        if (text2 == null)
        {
            throw new NullPointerException("text2 MUST NOT be null");
        }
        if (!caseSensitive)
        {
    ...
    public static int computeDistanceWord(String test, String reference)
    {
        ...
        test = UtilText.upperCaseWithoutAccent(test.replace("œ", "oe")
                                                   .replace("Œ", "OE")
                                                   .replace("æ", "ae")
                                                   .replace("Æ", "AE"));
        reference = UtilText.upperCaseWithoutAccent(reference.replace("œ", "oe")
                                                             .replace("Œ", "OE")
                                                             .replace("æ", "ae")
                                                             .replace("Æ", "AE"));
        int distance = 1;
     ...
     */
    public static JHelpFont computeMaximumSizedFont(final String text, final int maximumWidth, final int maximumHeight,
     ...
     * @param separator Separator where cut
     * @return Parts
     */
     ...
     *
     * @param string String where extract the doubles
    ...
    private static List<Pair<Integer, Integer>> extractRealsPair(final String string)
    {
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            if (((car == '-') && (start < 0)) || ((car == '.') && ((!havePoint) || (start == (i - 1)))) || ((car >= '0')
                                                                                                                    &&
                                                                                                                    (car <= '9')))
            {
                if (start < 0)
     ...
     *
     * @param string String where extract the integers
    ...
    /**
     * Return a list of indexes pairs. Each pair contains a start index and a end index. Each couple delimit where an
     * integer is
     * inside a string.
     *
     * @param string String where search all integers inside
     ...
     *
     * @param string String where extract the longs
    ...
    /**
     * Compute the index of a character in a string on ignoring characters between " or ' an in ignore characters with \
     * just
     * before them
     *
     * @param string    String where search
     * @param character Looking for character
     ...
     * @param string     String where search
     * @param character  Looking for character
     * @param startIndex Index where start to search
     * @return Index of character or -1 if not found
     */
     ...
     *
     * @param text           Text where search
     * @param search         Searched text
     * @param startIndex     Index where start the search
     * @param stringLimiters String limiters to use
                                           ...
                                           final String stringLimiters)
    {
        ...
        while ((index + lengthSearch) <= lengthText)
        {
            ...
            else if (Utilities.contains(character, limiters))
            {
                insideString = true;
    ...
    {
        final StringBuilder stringBuffer = new StringBuilder(string.length());
     ...
     * @return Index of the last character found in char sequence that inside in the given list. -1 if the char sequence
     * doesn't
    ...
    {
        final char[] characters = string.toUpperCase()
                                        .toCharArray();
        final int    length     = characters.length;
        if (length > 8)
        {
            throw new IllegalArgumentException(
                    UtilText.concatenate("The number in hexadecimal can't be more than 8 characters, so '", string,
                                         "' can't be converted"));
        }
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            {
                throw new IllegalArgumentException(
                        UtilText.concatenate("The string '", string, "' have illegal character at ", i));
            }
        }
    ...
    public static int parseInteger(final String string)
    {
        ...
        final char[] characters = string.substring(2)
                                        .toUpperCase()
                                        .toCharArray();
        final int    length     = characters.length;
        if (length > 8)
        {
            throw new IllegalArgumentException(
                    UtilText.concatenate("The number in hexadecimal can't be more than 8 characters (after the 0x), so '",
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            {
                throw new IllegalArgumentException(
                        UtilText.concatenate("The string '", string, "' have illegal character at ", i + 2));
            }
        }
    ...
    public static String putAntiSlash(final String string, final char... additionals)
    {
        final StringBuilder stringBuffer = new StringBuilder(string.length());
        final char[]        detect       = new char[additionals.length + 6];
        detect[0] = '\n';
        detect[1] = '\t';
        detect[2] = '\f';
        detect[3] = '\r';
        detect[4] = '\b';
        System.arraycopy(additionals, 0, detect, 6, additionals.length);
        int start = 0;
        int index = UtilText.indexOf(string, detect);
        ...
        while (index >= 0)
        {
            ...
            start = index + 1;
            index = UtilText.indexOf(string, start, detect);
        }
     ...
     * @param offset Start read index in array
    ...
    public static String repeat(final String string, final int time)
    {
        ...
        final StringBuilder stringBuffer = new StringBuilder(time * length);
        for (int i = 0; i < time; i++)
        {
     ...
     * holes are like {0}, {8}, ... That means that the first replacement replace every holes {0}, then ninth every {8},
     * ...<br>
     ...
     * </table>
     *
     * @param originalString String with "holes"
     * @param replacement    List of replacement for "holes". First replace hole {0}, second {1}, third {2} ,....
    ...
    public static String replaceHole(final String originalString, String... replacement)
    {
        ...
        {
            throw new NullPointerException("The originalString MUST NOT be null !");
        }
     ...
     * If the word just before the <b>{</b>, need to be change if the number is at least 2, then put after the hole
     * number the
     ...
     * <b>cat</b> and id the fourth number is 2 or more, the word will become <b>cats</b>. But it exist some case or just
     * add
     * letters is no enough, like <b>1 foot, 2 feet</b>. In this case, just put some <b>#</b> character after the <b>_</b>
     * followed by the letters to add. The number of last letter you want remove from the original word corresponds to
     * the number
     * of <b>#</b> to put. By example, for <b>foot</b> become <b>feet</b>, we have to remove the 3 last letters (So have
     * to put 3
     * <b>#</b>) then add letters <b>"eet"</b>, so have to write : <b>foot{2_###eet}</b>. That's means if third number is
     * under
     * 2, <b>foot</b> is write, and if third number is 2 or over, <b>foot</b> is write, and 3 last letters are removed
     * (Because
     ...
     *
     * @param originalString String with "holes"
     * @param values         List of values for "holes". First evaluate hole {0}, second {1}, third {2} ,....
     * @return Result string with replacement
     */
     ...
     *
     * @param text Text where replace white characters
     * @param c    Character used for replacement
     ...
     * </table>
     * .<br>
     * External link give possibility to get image relatively of directory where running jar is. So you can deploy
     * application
     ...
     * </table>
     * <br>
     * Other protocols with absolute path (file:..., jar:..., ...) or url (http:..., https:..., ...) are not modified and
     * copy as
     * they are
     *
    ...
    public static String resolveImagesLinkInHTML(String html)
    {
        ...
        while (matcher.find())
        {
            ...
            stringBuilder.append(matcher.group(1));
            stringBuilder.append("file:");
            stringBuilder.append(UtilIO.obtainExternalFile(matcher.group(2))
                                       .getAbsolutePath());
            start = matcher.end();
        }
    ...
    public static String toHexadecimal(final byte[] array)
    {
        ...
        for (byte anArray : array)
        {
            ...
            chars[indexWrite++] = Integer.toHexString((b >> 4) & 0xF)
                                         .charAt(0);
            chars[indexWrite++] = Integer.toHexString(b & 0xF)
                                         .charAt(0);
        }
     ...
     * @param charactersToRemove List of characters to remove
    ...
    public static String trim(final String stringToTrim, final String charactersToRemove)
    {
        ...
        while (start < end)
        {
            if (!Utilities.contains(characters[start], toRemove))
            {
                break;
            }
        ...
        while (start < end)
        {
            if (!Utilities.contains(characters[end - 1], toRemove))
            {
                break;
            }
