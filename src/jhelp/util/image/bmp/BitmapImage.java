...
public class BitmapImage
{
     ...
     *           Stream to read
     * @param jumpHeader
     *           Indicates if have to read to first part ({@code false}) of header (so all header read) OR not read the
     *           first
    ...
    public static BitmapImage parse(final InputStream inputStream) throws IOException
    {
        ...
        final int             height          = bitmapHeader.getHeight();
        ...
        switch (rasterImageType)
        {
            ...
            case IMAGE_4_BITS:
                ...
                break;
            case IMAGE_8_BITS:
                rasterImage = new Image8Bit(width, height);
                ...
                break;
            case IMAGE_16_BITS:
                rasterImage = new Image16Bit(width, height);
                ((Image16Bit) rasterImage).parseBitmapStream(inputStream);
                break;
            case IMAGE_24_BITS:
                rasterImage = new Image24Bit(width, height);
                ((Image24Bit) rasterImage).parseBitmapStream(inputStream);
                break;
            case IMAGE_32_BITS:
                rasterImage = new Image32Bit(width, height);
