...
import jhelp.util.samples.pcx.SamplePCX;
 ...
 * href="http://en.wikipedia.org/wiki/PCX">PCX image data info </a> and tests in {@link SamplePCX}
 * 
 * @author JHelp
...
public class PCX
{
   ...
   {
      // The palette 16 index is dispatch like that, lower bits first, upper bits last (only first 8 colors (0-7) of the palette
      // are used)
      final int[] scanLine = new int[this.scanLineSize];
   ...
   private void read256Palette(final InputStream inputStream) throws IOException
   {
      ...
      if(this.has256Palette == true)
      {
         ...
         {
            this.palette256[index] = 0xFF000000 | ((data[read] & 0xFF) << 16) | ((data[read + 1] & 0xFF) << 8) | (data[read + 2] & 0xFF);
         }
      }
   ...
   private void readHeader(final InputStream inputStream) throws IOException
   {
      ...
      {
         this.palette16[index] = 0xFF000000 | ((header[read] & 0xFF) << 16) | ((header[read + 1] & 0xFF) << 8) | (header[read + 2] & 0xFF);
      }
