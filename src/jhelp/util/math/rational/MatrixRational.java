...
public class MatrixRational
{
     ...
     *
     * @param xy x and y coordinate of diagonal
     */
    private void pivoteInternal(final int xy)
    {
        ...
        for (int yy = xy + 1; yy < this.height; yy++)
        {
            ...
            if (coeficient != Rational.ZERO)
            {
                ...
                {
                    this.matrix[destination] = this.matrix[destination].subtract(
                            coeficient.multiply(this.matrix[source++]));
                    destination++;
                }
            }
     ...
     *
     * @param matrixRational Matrix to add
     */
    ...
    public boolean equals(final Object object)
    {
        ...
        for (int i = 0; i < this.size; i++)
        {
            if (!this.matrix[i].equals(matrixRational.matrix[i]))
            {
                return false;
            }
        }
     ...
     *
     * @param matrixRational Multiplied matrix
     * @return Result
     */
    public MatrixRational multiplication(final MatrixRational matrixRational)
    {
        ...
        for (int y = 0; y < this.height; y++)
        {
            ...
            {
                (new TaskMultiplicationCell(count, x, y, this.width, matrixRational.width, matrixRational.width,
                                            result.matrix, this.matrix, matrixRational.matrix)).start();
            }
        }
     ...
     *
     * @param matrixRational Matrix to subtract
     */
