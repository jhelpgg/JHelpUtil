        ...
        implements Comparable<MayaNumber>
{
    ...
    static
    {
        ...
        try
        {
            font = JHelpFont.createFont(Type.TRUE_TYPE, MayaNumber.class.getResourceAsStream("Roboto-Regular-Maya.ttf"),
        ...
        try
        {
            font = JHelpFont.createFont(Type.TRUE_TYPE, MayaNumber.class.getResourceAsStream("Roboto-Regular-Numbers.ttf"),
                                        32, Value.FREE, Value.FREE, false);
        }
        catch (final Exception exception)
        {
     ...
     *
     * @param number Number to convert in Maya
     ...
     * String MUST only contains defined digits and MUST NOT be {@code null} or empty
     *
     * @param toParse String to parse in Maya number
     ...
     * {@link MayaNumber#DIGIT_17_UUCLAHUN}, {@link MayaNumber#DIGIT_18_UAXACLAHUN} and
     * {@link MayaNumber#DIGIT_19_BOLONLAHUN}
     *
     * @param digits List of defined digits to represents a Maya number
     * @throws NullPointerException     if the list is {@code null}
     * @throws IllegalArgumentException If the list is empty OR Or of given digit is not a define one
     */
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            if ((digit < MayaNumber.DIGIT_00_MIL) || (digit > MayaNumber.DIGIT_19_BOLONLAHUN))
            {
                throw new IllegalArgumentException(
                        UtilText.concatenate("Given digits contains at least one illegal character at index ", i,
                                             " digits=", digits));
            }
     ...
     * {@inheritDoc}
     *
     * @param mayaNumber Maya number to compare with
     ...
     * @param horizontal Indicates if toem are put horizontally ({@code true}) OR vertically ({@code false})
    ...
    public JHelpImage toTotemImage(final int baseSize, final boolean horizontal)
    {
        ...
        for (byte aBase20 : this.base20)
        {
            try
            {
                image = JHelpImage.loadImageThumb(
                        MayaNumber.class.getResourceAsStream(MayaNumber.TOTEM_NUMBERS[aBase20 & 0xFF]), baseSize,
                        baseSize);
                totem.drawImage(x, y, image, false);
            }
            catch (final Exception exception)
            {
                Debug.printException(exception, "Failed to get image for number ", aBase20 & 0xFF);
            }
