...
import jhelp.util.samples.math.maya.SampleTribal;
 ...
 * It can be "0" (Represents by a big dot), or their have one | per value, but for 5, a group representation is used instead of
 * 5 |. Same thing happens for 25 a 25 representation is used instead of 5 "5 symbol". To follow this logic, there also a
 * representation for 125 and 625.<br>
 * After is bit more complicated, the symbols before the 625 symbol (*) are multiplier of 625 (The much time 625) By example
 * 1250 is represented : ||* (Means 2 times 625), but numbers after the 625 symbol are just add, by example 1252 is ||*||
 * (2*625+2). For a better understanding try {@link SampleTribal} (Type numbers and then enter to see tribal representation)
 * 
 * @author JHelp
 */
public class TribalNumber
{
    ...
    * 
    * @param characters
    *           Characters to create tribal number
    * @return Created tribal number
    * @throws IllegalArgumentException
    *            If one of character is not a tribal number character
    */
   ...
   public TribalNumber(long number)
   {
      if(number < 0)
      {
         throw new IllegalArgumentException("Tribal didn't know negative numbers like " + number);
      }
