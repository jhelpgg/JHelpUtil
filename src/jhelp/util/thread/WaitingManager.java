...
public class WaitingManager
{
     ...
     * @param <PROGRESS>   Task progress type
     * @param threadedTask Task that wait
     * @param parameter    Parameter to give to task when its time to play comes
     * @param waitingFor   Waiting event
     */
    public <PARAMETER, RESULT, PROGRESS> void register(final ThreadedTask<PARAMETER, RESULT, PROGRESS> threadedTask,
                                                       final PARAMETER parameter, final String waitingFor)
    {
        if (threadedTask == null)
        {
            throw new NullPointerException("threadedTask MUST NOT be null");
        }

        if (waitingFor == null)
        {
            throw new NullPointerException("waitingFor MUST NOT be null");
        }
     ...
     * @param <RESULT>    Task result type
     * @param <PROGRESS>  Task progress type
     * @param waitingFor  Event to wake up
     */
        ...
        try
        {
            ...
            if (queue != null)
            {
                while (!queue.isEmpty())
                {
                    ...
                    ThreadManager.THREAD_MANAGER.doThread((ThreadedTask<PARAMETER, RESULT, PROGRESS>) pair.element1,
                                                          (PARAMETER) pair.element2);
                }
