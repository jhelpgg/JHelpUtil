        ...
        implements Iterable<TYPE>
{
                       ...
                       final boolean unique)
    {
        ...
        if (comparator == null)
        {
            if (Reflector.isSubTypeOf(typeClass, Comparable.class))
            {
    ...
    public TYPE seekElement(final SeekTest<TYPE> seekTest)
    {
        for (int index = 0; index < this.size; index++)
        {
            if (seekTest.isElementSeek(this.array[index]))
            {
                return this.array[index];
            }
        }
    ...
    {
        @SuppressWarnings("unchecked")
        final SortedArray<TYPE> sortedArray = new SortedArray<TYPE>(this.typeClass, this.comparator, this.size,
                                                                    this.unique);
