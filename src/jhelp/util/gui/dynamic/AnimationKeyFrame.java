 ...
 * This animation types say sat this frame the object must have this value and interpolate the values if the frame is
 * between 2
 * defined frame.<br>
        ...
        implements DynamicAnimation
{
    ...
    public final boolean animate(final float absoluteFrame, final JHelpImage image)
    {
        synchronized (this.keyFrames)
        {
            ...
            frame = 0;
            while ((frame < size) && (this.keyFrames.getElement(frame).frame < actualFrame))
            {
                frame++;
            }
