...
/**
 * Way to call distant method between two Java applications in same computer.<br>
 ...
 * 1) All referred class, interfaces, enum or exception must be know by sender and receiver.<br>
...
{
    /** Pipe main directory (Where lies all pipes, see {@link PipeReader} and {@link PipeWriter} for know more about
     * pipes */
    ...
    static
    {
        ...
        final String[] paths =
                {
                        System.getProperty("user.home"), System.getProperty("user.dir"), System.getProperty(
                        "java.home"), UtilIO.obtainOutsideDirectory().getAbsolutePath()
                };
    ...
    private static List<NameCode> createClassesForInterface(final Class<?> interfaceIDL)
    {
        ...
        final String  packageName  = interfaceIDL.getPackage()
                                                 .getName();
        final String  simpleName   = interfaceIDL.getSimpleName();
        final String  completeName = interfaceIDL.getName();
        StringBuilder code;
        String        name;
        ...
        for (final Method method : methods)
        {
            ...
            if ((claz != null) && (claz.isArray()))
            {
                claz = claz.getComponentType();
            }

            if ((claz != null) && (!claz.isPrimitive()) && (!claz.getSimpleName()
        ...
        name = completeName + JHelpIDL.COMMON;
        ...
        for (final Method method : methods)
        {
            ...
            claz = method.getReturnType();
            if ((claz != null) && (!claz.getSimpleName()
                                        .equalsIgnoreCase("void")))
            {
        ...
        code = new StringBuilder();
        name = completeName + JHelpIDL.REQUEST;
        ...
        code = new StringBuilder();
        name = completeName + JHelpIDL.ANSWER;
        ...
        for (final Method method : methods)
        {
            ...
            if ((returnType == null) || (returnType.getSimpleName()
                                                   .equalsIgnoreCase("void")))
            {
                continue;
            }
        ...
        // Create source
        code = new StringBuilder();
        name = completeName + JHelpIDL.SOURCE;
        ...
        code.append("   private final PipeWriter pipeWriterRequest = new PipeWriter(PIPE_DIRECTORY_REQUEST);\n");
        ...
        for (final Method method : methods)
        {
            ...
            code.append(");\n         pipeWriterRequest.write(request);\n");
                                                    ...
                                                    .equalsIgnoreCase("void")))
            {
                ...
                if (JHelpIDL.needClassAsReadParameter(returnType))
                {
                    code.append(returnType.getSimpleName());
                    code.append(".class");
                }
        ...
        // Create target
        code = new StringBuilder();
        name = completeName + JHelpIDL.TARGET;
        ...
        code.append("   private final PipeWriter pipeWriterAnswer = new PipeWriter(PIPE_DIRECTORY_ANSWER);\n");
        ...
        for (final Method method : methods)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                if (JHelpIDL.needClassAsReadParameter(type))
                {
                    code.append(type.getSimpleName());
                    code.append(".class");
                }
    ...
    private static boolean needClassAsReadParameter(final Class<?> type)
    {
        ...
        if (Reflector.isInherit(type, String.class))
        {
            return false;
        }

        if (Reflector.isInherit(type, Binarizable.class))
        {
            return true;
        }
     ...
     * @param type
     *           Type to read
     * @return Read or write extension
     */
    ...
    public static void declareIDLs(final Class<?>... interfacesIDL) throws IOException
    {
        ...
        Compiler.compile(JHelpIDL.COMPILE_DIRECTORY_BIN, codes.toArray(new NameCode[codes.size()]));

        for (final NameCode nameCode : codes)
        {
            JHelpIDL.CLASS_LOADER.add(new File(JHelpIDL.COMPILE_DIRECTORY_BIN, nameCode.getName()
                                                                                       .replace('.',
                                                                                                File.separatorChar) +
                    JHelpIDL.CLASS));
        }
     ...
     */
    @SuppressWarnings("unchecked")
    public static <TYPE> void registerReceiver(final TYPE interfaceIDLImplementation, final Class<TYPE> interfaceIDL)
            throws ClassNotFoundException,
